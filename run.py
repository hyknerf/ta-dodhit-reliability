...
for i in range(len(df.columns)):
    ...
    if error == False:
        ...
        output = Fit_Everything(failures=df.iloc[:, i].dropna(
        ).tolist(), show_probability_plot=False, show_PP_plot=False)
        ...
        if output.best_distribution_name == 'Beta_2P':
            while t > 1:
                print(
                    "Error: for Beta distributions the range of the values must be within 0 and 1.")
                t = float(input("Type in the desired time before failure: "))

        # Append failure time in times listx
        times.append(t)
